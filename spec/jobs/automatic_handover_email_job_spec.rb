...
RSpec.describe AutomaticHandoverEmailJob, type: :job do
  ...
  shared_context 'with expected behaviour' do
    ...
    context 'with some offenders' do
      ...
      let(:offender1_model) { create(:offender) }
      let(:prison1) { create(:prison) }
      let(:case_info1) { build(:case_information, nomis_offender_id: offender1_model.nomis_offender_id) }
      let!(:allocation1) { create(:allocation_history, prison: prison1.code, nomis_offender_id: offender1_model.nomis_offender_id, primary_pom_nomis_id: staff_id) }
      let(:offender1) do
        build(:nomis_offender, prisonId: prison1.code, prisonerNumber: offender1_model.nomis_offender_id, firstName: 'One',
      ...
      let!(:handover1) do
        CalculatedHandoverDate.create!(
          nomis_offender_id: offender1_model.nomis_offender_id,
          handover_date: 8.days.from_now,
      ...
      let(:offender2_model) { create(:offender) }
      let(:case_info2) { build(:case_information, nomis_offender_id: offender2_model.nomis_offender_id) }
      let(:offender2) do
        build(:nomis_offender, prisonId: prison1.code, prisonerNumber: offender2_model.nomis_offender_id, firstName: 'Two',
                               sentence: attributes_for(:sentence_detail, :handover_in_4_days, conditionalReleaseDate: Time.zone.today + 19.days + 8.months))
      end
      let!(:handover2) do
        CalculatedHandoverDate.create!(
          nomis_offender_id: offender2_model.nomis_offender_id,
          handover_date: 4.days.from_now,
      ...
      let(:offender3_model) { create(:offender) }
      let(:case_info3) { build(:case_information, nomis_offender_id: offender3_model.nomis_offender_id) }
      let!(:allocation3) { create(:allocation_history, prison: prison3.code, nomis_offender_id: offender3_model.nomis_offender_id, primary_pom_nomis_id: staff_id) }
      let(:offender3) do
        build(:nomis_offender, prisonId: prison3.code, prisonerNumber: offender3_model.nomis_offender_id, firstName: 'Three',
                               sentence: attributes_for(:sentence_detail, :handover_in_6_days, conditionalReleaseDate: Time.zone.today + 21.days + 8.months))
      end
      let!(:handover3) do
        CalculatedHandoverDate.create!(
          nomis_offender_id: offender3_model.nomis_offender_id,
          handover_date: 6.days.from_now,
      ...
      let(:offender6_model) { create(:offender) }
      let(:case_info6) { build(:case_information, nomis_offender_id: offender6_model.nomis_offender_id) }
      let!(:allocation6) { create(:allocation_history, :release, prison: prison6.code, nomis_offender_id: offender6_model.nomis_offender_id) }
      let(:offender6) do
        build(:nomis_offender, prisonId: prison6.code, prisonerNumber: offender6_model.nomis_offender_id, firstName: 'Six',
                               sentence: attributes_for(:sentence_detail, :handover_in_3_days, conditionalReleaseDate: Time.zone.today + 18.days + 8.months))
      end
      let!(:handover6) do
        CalculatedHandoverDate.create!(
          nomis_offender_id: offender6_model.nomis_offender_id,
          handover_date: 3.days.from_now,
    ...
    def handover_fields(offset)
      [today_plus(offset), today_plus(8.months + offset + 15.days)]
    end

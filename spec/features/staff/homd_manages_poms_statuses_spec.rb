describe 'HOMD manages POMS statuses' do
  ...
  specify 'edit page shows the "No longer at prison" option' do
    # Given the POM is active
    create(:pom_detail, :active, nomis_staff_id: 1234, prison:)
    ...
    within('fieldset', text: 'Select status') do
      ...
      expect(page).to have_content("No longer at #{prison.name}")
    end
  end

  specify 'HOMD making a POM inactive with no cases moves them to away from work' do
    # Given the POM is active
    create(:pom_detail, :active, nomis_staff_id: 1234, prison:)

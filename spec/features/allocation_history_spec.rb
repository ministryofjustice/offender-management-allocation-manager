...
feature 'Allocation History' do
  ...
  scenario 'view offender allocation history', versioning: true, vcr: { cassette_name: :offender_allocation_history } do
    ...
    stub_const("TESTS", [
        ...
        ['p', "Prisoner reallocated to #{history1.primary_pom_name.titleize} - (email address not found) Tier: #{history1.allocated_at_tier}"],
        ...
        ['.govuk-heading-s', "Prisoner reallocated"],
        ['p', "Prisoner reallocated to #{history6.primary_pom_name.titleize} - #{probation_pom_2[:email]} Tier: #{history6.allocated_at_tier}"],

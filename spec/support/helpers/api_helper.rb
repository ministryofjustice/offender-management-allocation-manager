  ...
  def stub_offenders_for_prison(prison, offenders, movements = [])
    ...
    stub_request(:get, elite2listapi).with(
      headers: {
        'Page-Limit' => '200',
        'Page-Offset' => '0'
      }).to_return(body: offenders.zip(booking_ids).map { |o, booking_id| o.except(:sentence, :recall).merge('bookingId' => booking_id, 'agencyId' => prison, 'assignedLivingUnitDesc' => o[:internalLocation]) }.to_json)

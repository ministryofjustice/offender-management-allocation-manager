RSpec.describe Handover::HandoverEmail do
  ...
  let(:mailer_with) { double(:mailer_with, upcoming_handover_window: mailer) }

  before do
    allow(HandoverMailer).to receive(:with).and_return(mailer_with)
  ...
  describe "::deliver_if_deliverable" do
    describe "when not opted out and not already sent" do
      ...
      it "sends the email" do
        aggregate_failures do
          expect(HandoverMailer).to have_received(:with).with(**final_mailer_args)
    ...
    it "works with other email types" do
      allow(mailer_with).to receive_messages(handover_date: double.as_null_object)
      described_class.deliver_if_deliverable(:handover_date, nomis_offender_id, pom_id, email: pom_email)
      expect(mailer_with).to have_received(:handover_date)
    end
  end
end

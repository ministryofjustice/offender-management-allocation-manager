RSpec.describe HandoverCasesList do
  ...
  describe '#upcoming' do
    it 'gets a list of handover cases whose handovers are upcoming' do
      expect(handover_cases_list.upcoming).to eq([[upcoming_calculated_handover_dates[0], allocated_offenders['A']],
                                                  [upcoming_calculated_handover_dates[1], allocated_offenders['C']]])
    end

    it 'does not include upcoming handover cases with a COM allocated' do
      allow(allocated_offenders['C']).to receive(:allocated_com_name).and_return('TEST COM NAME')

      expect(handover_cases_list.upcoming.map(&:second)).not_to include(allocated_offenders['C'])
    end
  end

  describe '#in_progress' do
    it 'gets a list of handover cases whose handovers are in progress' do
      expect(handover_cases_list.in_progress).to eq([[in_progress_calculated_handover_dates[0],
                                                      allocated_offenders['D']],
                                                     [in_progress_calculated_handover_dates[1],
                                                      allocated_offenders['F']]])
    end

    it 'includes upcoming handover cases that have a COM allocated' do
      allow(allocated_offenders['C']).to receive(:allocated_com_name).and_return('TEST COM NAME')

      expect(handover_cases_list.in_progress.map(&:second)).to include(allocated_offenders['C'])
    end
  end
end

...
namespace :reports do
  desc 'Create a CSV report for all currently valid allocations regardless of responsibility'
  task allocations: :environment do
    ...
    def pom_type_columns(pom_types, allocation, responsibility)
      primary_pom_type = pom_types[allocation.primary_pom_nomis_id]
      secondary_pom_type = allocation.secondary_pom_nomis_id.present? ? pom_types[allocation.secondary_pom_nomis_id] : nil

      responsible = nil
      supporting = nil

      if responsibility&.pom_responsible?
        # CustodyOnly or CustodyWithCom: POM is responsible
        responsible = primary_pom_type
      elsif responsibility&.com_responsible?
        # Community: COM is responsible, POM is supporting
        supporting = primary_pom_type
      end

      [responsible, supporting, secondary_pom_type]
    end
    ...
    CSV.open(Reports::TaskOptions.filename('allocations.csv'), 'wb') do |csv|
      ...
      prisons.each do |prison|
        log ">> Obtaining current allocations for #{prison.name} (#{prison.code})"
